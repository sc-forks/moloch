...
contract Moloch {
    ...
    *****************/
        ...
        onlyMember
    {
        ...
        Proposal storage proposal = Proposal(msg.sender, applicant, votingSharesRequested, startingPeriod, 0, 0, tributeTokenAddresses, tributeTokenAmounts, false);
    ...
    function processProposal(uint256 proposalIndex) public {
        ...
        if (proposal.yesVotes.add(proposal.noVotes) >= (totalVotingShares.mul(QUORUM_NUMERATOR)).div(QUORUM_DENOMINATOR) && proposal.yesVotes > proposal.noVotes) {
            ...
            for (uint256 i=0; i < proposal.tributeTokenAddresses.length; i++) {
                require(guildBank.depositTributeTokens(this, proposal.tributeTokenAddresses[i], proposal.tributeTokenAmounts[i]));
            }
        } else {
            // return all tokens
            for (uint256 j=0; j < proposal.tributeTokenAddresses.length; j++) {
                ERC20 token = ERC20(proposal.tributeTokenAddresses[j]);
                require(token.transfer(proposal.applicant, proposal.tributeTokenAmounts[j]));
            }
        }
